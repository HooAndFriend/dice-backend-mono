if [ $# -eq 0 ]; then
    ...
    ssh -t dice "sh restart.sh nginx"
    sleep 5
    curl --location "https://api.hi-dice.com/api/slack/v1/slack" --header "Content-Type:application/json" --data '{"name":"BACKEND:AUTH-SERVER","code":"MAS","message":"BACKEND:AUTH-SERVER 배포가 완료되었습니다."}'
elif [ $1 = "log" ]; then
    ...
    ssh -t dice "sh restart.sh nginx"
    sleep 5
    curl --location "https://api.hi-dice.com/api/slack/v1/slack" --header "Content-Type:application/json" --data '{"name":"BACKEND:ADMIN-SERVER","code":"MAS","message":"BACKEND:ADMIN-SERVER 배포가 완료되었습니다."}'
elif [ $1 = "file" ]; then
